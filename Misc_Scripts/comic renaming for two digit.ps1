...
$newString = Read-Host "Enter the string to replace the filenames (except the two-digit number)"

# Get all JPG files in the folder
$jpgFiles = Get-ChildItem -Path $folderPath -Filter "*.jpg"

# Regular expression pattern to match filenames ending in a two-digit number
$pattern = "^(.*\D)?(?<number>\d{2})\.jpg$"
