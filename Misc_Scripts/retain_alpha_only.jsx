    ...
    for (var i = 0; i < files.length; i++) {
        ...
        if (file instanceof File) {
            ...
            if (doc.channels.length > 3) {
                ...
                var newDoc = app.documents.add(doc.width, doc.height, doc.resolution, file.name.replace(/\.tga$/, "_alpha"), NewDocumentMode.GRAYSCALE);
                app.activeDocument = newDoc;
                
                // Paste the alpha channel as the new image
                newDoc.paste();
                ...
                // Save as PNG
                var saveFile = new File(outputFolder + "/" + file.name.replace(/\.tga$/, "_alpha.png"));
